...
{
    module = torch::jit::load("/home/rvclab/ORB_SLAM3/yolov5s.torchscript.pt");
    torch::jit::setTensorExprFuserEnabled(false);
    std::ifstream f("/home/rvclab/ORB_SLAM3/coco.names");
    std::string name = "";
...
void YOLOv5Detector::Run(){
    ...
    while(1)
    {
        // std::cout << "[INFO] in while---------------------------------------------------" << endl;
        // while ( mpFrameDrawer->CheckImageStatus() == false )
        // {
        //     std::cout << "[INFO] in dead loop---------------------------------------------------" << endl;
        // }
        // std::cout << "[INFO] CheckImageStatus finished------------------------------------------------" << endl;
        mImage = mpFrameDrawer->GetFrame();
        // std::cout << "channels: " << mImage.channels() << endl;
        // std::cout << "data type: " << mImage.type() << endl;
        // cv::cvtColor(mImage, mImage, cv::COLOR_GRAY2BGR);
        cv::Mat img;
        ...
        // cv::namedWindow("ORB-SLAM3: YOLOv5Detector");
        // cv::imshow("ORB-SLAM3: YOLOv5Detector", showImage);
        if (cv::waitKey(1) == 27)
            break;
        // -------------------------------------------------------
