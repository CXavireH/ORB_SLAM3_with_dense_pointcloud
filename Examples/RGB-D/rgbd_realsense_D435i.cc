...
int main(int argc, char **argv)
{
    ...
    {
        cerr << endl
             << "Usage: ./rgbd_realsense_D435i path_to_vocabulary path_to_settings (trajectory_file_name)"
             << endl;
        return 1;
    }

    string file_name;            //path of the trajectory_file_name
    ...
    /*
    注册一个信号处理函数来处理SIGINT（中断信号）的信号。在接收到SIGINT信号时，将调用名为"exit_loop_handler"的函数。
    */
    ...
    /*
    这段代码使用Intel RealSense SDK（RS2）库来获取连接到计算机上的RealSense设备的信息。
    可以用sensors变量进一步操作这些传感器
    */
    rs2::context ctx;
    ...
    */
    ...
    for (rs2::sensor sensor:sensors)
        if (sensor.supports(RS2_CAMERA_INFO_NAME)) 
        {
            ++index;
            if (index == 1) {
                // 由于D455相机设备不支持代码中的 AUTO_EXPOSURE 相关选项导致bug，所以注释下面两行
                // sensor.set_option(RS2_OPTION_ENABLE_AUTO_EXPOSURE, 1);
                // sensor.set_option(RS2_OPTION_AUTO_EXPOSURE_LIMIT,50000);
                sensor.set_option(RS2_OPTION_EMITTER_ENABLED, 1); // emitter on for depth information启用深度信息的红外发射器
            }
            /*
                1 : Stereo Module
                2 : RGB Camera
            ...
            */
            ...
            if (index == 2){
                // RGB camera
                sensor.set_option(RS2_OPTION_EXPOSURE,80.f);//设置RGB相机的曝光值为80。
            }
            if (index == 3){
                sensor.set_option(RS2_OPTION_ENABLE_MOTION_CORRECTION,0);//用于禁用运动校正。
            }
        }
    // Declare RealSense pipeline, encapsulating the actual device and sensors
    /*
    创建pipeline对象，可以方便地配置和控制与RealSense设备之间的数据传输和处理。
    pipeline对象提供了一种简化的接口，用于启动、停止和管理数据流，并提供了访问传感器数据的方法。
    */
    rs2::pipeline pipe;
    ...
    RS2_STREAM_INFRARED：指定数据流的类型为红外图像流。
    1：指定数据流的索引为1。如果RealSense设备支持多个红外传感器，可以通过索引来选择具体的传感器。
    640和480：指定数据流的分辨率为宽度640像素和高度480像素。
    RS2_FORMAT_Y8：指定数据流的像素格式为Y8，表示每个像素由一个8位的灰度值组成。
    30：指定数据流的帧率为30帧/秒。

    RS2_STREAM_DEPTH：指定数据流的类型为深度图像流。
    640和480：指定数据流的分辨率为宽度640像素和高度480像素。
    RS2_FORMAT_Z16：指定数据流的像素格式为Z16，表示每个像素由一个16位的深度值（以毫米为单位）组成。
    30：指定数据流的帧率为30帧/秒
    ...
    */
    ...
    1  std::mutex imu_mutex; 定义了一个名为imu_mutex的互斥锁对象。
    互斥锁用于在多线程环境中实现对共享资源的互斥访问，以防止多个线程同时访问和修改共享资源而导致数据竞争。
    

    2  std::condition_variable cond_image_rec; 定义了一个名为cond_image_rec的条件变量对象。
    条件变量用于在线程间进行同步和通信，允许一个或多个线程等待特定条件的发生并进行相应的操作。
    */
    std::mutex imu_mutex;
