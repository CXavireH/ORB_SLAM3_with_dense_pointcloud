...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc,
                   ...
                   shared_ptr<PointCloudMapping> pPointCloud, const string &_nameSeq):
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if (!bOK)
                {
                    ...
                    {
                        cout << "Recentlost---KF in map: " << pCurrentMap->KeyFramesInMap() << endl;
            ...
            else
            {
                if (mState == RECENTLY_LOST)
                {
                    ...
                    {
                        if(pCurrentMap->isImuInitialized())  //如果imu初始化完成，
                            PredictStateIMU();               //用imu计算位姿。
                        else
                            bOK = false;
                        if (mCurrentFrame.mTimeStamp-mTimeStampLost>time_recently_lost)
                        {   
                            // 如果lost时间大于阈值time_recently_lost（5s），则lost
                            mState = LOST;
                ...
                else if (mState == LOST)
                {
                    ...
                    // 如果地图中关键帧的数量小于10，则重置地图，否则新建一个地图。
        ...
        else
        {
            ...
            // TODO: Not available in inertial mode
            if(mState==LOST)
            {
                if(mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
            ...
            {
                if(!mbVO)
                // mbVO为false时跟踪正常，为true时没有点match to current map，即跟不上了。
                {
                    // In last frame we tracked enough MapPoints in the map
                    if(mbVelocity)
                    // 恒速模型非空，用恒速模型，否则跟踪参考关键帧
                    {
                        bOK = TrackWithMotionModel();
                    }
                    else
        ...
        if(!mbOnlyTracking)
        // 正常模式下，如果跟踪成功，则跟踪局部地图，优化当前帧的位姿 TrackLocalMap()
        {
            if(bOK)
            {
                bOK = TrackLocalMap();
